...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Move one trip.services to trip.service"
        issue = False
        for trip in orm['multigtfs.trip'].objects.filter(service__isnull=True):
            if trip.services.count() == 1:
                trip.service = trip.services.get()
                trip.save()
            else:
                print(
                    'Trip {} has {} services, should have exactly 1'.format(
                        trip.id, trip.services.count()))
                issue = True
        if issue:
            raise RuntimeError("Fix trips and retry migration")

    def backwards(self, orm):
        "Move trip.service back to trip.services"
        for trip in orm['multigtfs.trip'].objects.all():
            trip.services.add(trip.service)
    ...
    models = {
        ...
        u'multigtfs.trip': {
            ...
            'services': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "u'+'", 'symmetrical': 'False', 'to': u"orm['multigtfs.Service']"}),
