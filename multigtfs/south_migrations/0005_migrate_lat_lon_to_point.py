...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for stop in orm['multigtfs.stop'].objects.all():
            stop.point = 'POINT(%s %s)'%(stop.lon, stop.lat)
            stop.save()
        for shape_pt in orm['multigtfs.shapepoint'].objects.all():
            shape_pt.point = 'POINT(%s %s)'%(shape_pt.lon, shape_pt.lat)
            shape_pt.save()

    def backwards(self, orm):
        """Convert points to lat/lon"""
        for stop in orm['multigtfs.stop'].objects.all():
            stop.lon, stop.lat = stop.point.coords
            stop.save()
        for shape_pt in orm['multigtfs.shapepoint'].objects.all():
            shape_pt.lon, shape_pt.lat = shape_pt.coords
            shape_pt.save()
